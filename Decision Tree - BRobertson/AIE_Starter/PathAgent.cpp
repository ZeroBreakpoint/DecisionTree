...
// Updates the agent's movement along the path
...
// Starts pathfinding to a target node using Dijkstra's algorithm
void PathAgent::GoToNode(AIForGames::Node* end, NodeMap& nodeMap)
{
    if (end == nullptr) {
    ...
    // Calculate the path from the current node to the target node
    m_path = nodeMap.DijkstrasSearch(m_position, end);
    m_currentIndex = 0;  // Reset the current index for following the new path

}

// Draws the agent on the screen as a yellow circle
void PathAgent::Draw() const
{   //Draw Agent - set size and color to yellow
    DrawCircle((int)m_position.x, (int)m_position.y, 8, { 255, 255, 0, 255 });  // Draw agent as a yellow circle
}

// Clears the agent's current path
