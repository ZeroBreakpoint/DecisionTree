...
public:
    ...
    State(Behaviour* behaviour) // Modified constructor to accept a Behaviour as an argument
    {
    ...
    void AddTransition(Condition* condition, State* targetState)  // Function to add transitions to the state
    {
        m_transitions.push_back({ condition, targetState });
    }

    
    const std::vector<Transition>& GetTransitions() const { return m_transitions; }  // Accessor for transitions
